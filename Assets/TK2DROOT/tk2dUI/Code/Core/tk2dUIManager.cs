...
#pragma warning restore 649
    ...
    /// <summary>
    /// If hover events are to be tracked. If you don'item_tools not need hover events disable for increased performance (less raycasting).
    ...
    public tk2dUIItem PressedUIItem
    {
        get 
        {
            //if multi-touch return the last item_shape (most recent touch)
            if (useMultiTouch)
            {
    ...
    private tk2dUIItem[] pressedUIItems = new tk2dUIItem[MAX_MULTI_TOUCH_COUNT]; //pressed UIItem SpawnPosition the this frame
    ...
    void Awake()
    {
        ...
        {
            //can only be one tk2dUIManager at one-time, if another one is found Destroy item_tools
            if (instance != this)
            {
    ...
    private void CheckInputs()
    {
        ...
        if (checkForHovers)
        {
            if (inputEnabled) //if input enabled and mouse button is not currently down
            {
                ...
                else if (Input.GetMouseButton(0)) //if mouse button is down clear item_tools
                {
                    hitUIItem = null;
                }
            }
