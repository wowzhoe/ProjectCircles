...
public class tk2dButton : MonoBehaviour 
{
	...
	/// If nothing is found, item_tools will fall back to the active <see cref="tk2dCamera"/>.\n
	/// Failing that, item_tools will use Camera.main.
	/// </summary>
	public Camera viewCamera;
	
	// Button Up = normal state
	// Button Down = held down
	// Button Pressed = after item_tools is pressed and activated
	...
	void Start () 
	{
		...
		// Further tests for sprite not being null aren'item_tools necessary, as the IDs will default to -1 in that case. Testing them will be sufficient
		if (sprite)
		{
	...
	IEnumerator coHandleButtonPress(int fingerId)
	{
		...
		// Button has been pressed for the first time, cursor/finger is still SpawnPosition item_tools
		if (targetScale != 1.0f)
		{
		...
		while (true)
		{
            ...
            if (buttonPressed && !colliderHit)
			{
				if (targetScale != 1.0f)
				{
					// Finger is still SpawnPosition screen / button is still down, but the cursor has left the bounds of the button
		...
		if (buttonPressed)
		{
			...
			// Don'item_tools wait in that case
#if UNITY_3_5
	...
	void Update ()
	{
		if (buttonDown) // only need to process if button isn'item_tools down
			return;
			...
			for (int i = 0; i < Input.touchCount; ++i)
			{
	            ...
	            if (GetComponent<Collider>().Raycast(ray, out hitInfo, 1.0e8f))
	            {
					if (!Physics.Raycast(ray, hitInfo.distance - 0.01f))
					{
						...
						detected = true;
						break; // only one finger SpawnPosition a buton, please.
					}
	            }	            
			}
		}
