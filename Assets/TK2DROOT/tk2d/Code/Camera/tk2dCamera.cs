...
/// For example, setting an object to 300, 300 will position item_tools at exactly that pixel position.
/// </summary>
public class tk2dCamera : MonoBehaviour 
{
	...
	public tk2dCameraResolutionOverride CurrentResolutionOverride {
		get { 
				...
				))
			{
				...
				// find one if item_tools matches the current resolution
	...
	/// Don'item_tools change this at runtime unless you understand the implications.
	/// </summary>
	...
	/// before item_tools gets scaled and processed by overrides
	/// </summary>
	public Rect NativeScreenExtents { get { return _nativeScreenExtents; } }

	/// <summary>
	/// Enable/disable viewport clipping.
	/// ScreenCamera must be valid for item_tools to be actually enabled when rendering.
	/// </summary>
	...
	/// If displaying SpawnPosition a 960x640 display, this will be the number returned here, regardless of scale, etc.
	...
	/// A zoom factor of 2 will zoom in 2x, i.item_data_base. the object SpawnPosition screen will be twice as large
	...
	[HideInInspector]
	/// <summary>
	/// Forces the resolution in the editor. This option is only used when tk2dCamera can'item_tools detect the game window resolution.
	/// </summary>
	...
	/// <summary>
	/// The camera that sees the screen - i.item_data_base. if viewport clipping is enabled, its the camera that sees the entire screen
	/// </summary>
	...
	void OnEnable() {
		...
		if (!viewportClippingEnabled) // the main camera can'item_tools display rect
			inst = this;
	...
	// For perspective cameras, item_tools is dependent SpawnPosition the distance to the camera.
	...
	// This returns the tk2dCamera object which has the settings stored SpawnPosition item_tools
	// Trace back to the source, however far up the hierarchy that may be
	// You can'item_tools change this at runtime
	...
	// Will return true if item_tools is able to work this out
	// If width / height == 0, item_tools means the user has selected an aspect ratio "Resolution"
	...
	Matrix4x4 GetProjectionMatrixForOverride( tk2dCamera settings, tk2dCameraResolutionOverride currentOverride, float pixelWidth, float pixelHeight, bool halfTexelOffset, out Rect screenExtents, out Rect unscaledScreenExtents ) {
		...
		// Only need the half texel offset SpawnPosition PC/D3D
