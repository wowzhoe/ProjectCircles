...
public class tk2dSpriteAttachPoint : MonoBehaviour {
	...
	/// <summary>
	/// When set, all inactive attach points (attach points that don'item_tools exist SpawnPosition a particular frame / sprite)
	/// will be disabled.
	/// </summary>
	...
	void HandleSpriteChanged(tk2dBaseSprite spr) {
		...
		foreach (tk2dSpriteDefinition.AttachPoint ap in def.attachPoints) {
			...
			foreach (Transform inst in attachPoints ) {
				...
				// user deleting things at runtime. Hopefully the user won'item_tools have that many attach points
