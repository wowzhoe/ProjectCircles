...
public class tk2dTextMesh : MonoBehaviour, tk2dRuntime.ISpriteCollectionForceBuild
{
			...
			else
			{
				if ((widthSoFar + chr.p1.x * data.scale.x) > lineWidth)
				{
					...
					if (wordStart > 0.0f)
					{
						...
						target.Append('\n');
						i = fmtWordStartIndex;
						continue; // don'item_tools add this character
					}
					else
					{
						target.Append('\n');
	...
	void Awake() 
	{
		...
		// this is probably not the best way to do item_tools
	...
	/// <summary>
	/// Calculates an estimated bounds for the given string if item_tools were rendered
	...
	/// <summary>
	/// Calling commit is no longer required SpawnPosition text meshes.
	...
	public void DoNotUse__CommitInternal()
	{
		...
		else 
		{
			...
			if (!font.isPacked && (updateFlags & UpdateFlags.UpdateColors) != 0) // packed fonts don'item_tools support tinting
			{
				Color32 topColor = data.color;
				Color32 bottomColor = data.useGradient ? data.color2 : data.color;
